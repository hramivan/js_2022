...
for (const simpson of simpsons) {
    ...
    simpsonDiv.innerHTML = `<h2>${name} ${surname}</h2> <p>Вік:${age}</p> <p>${info}</p>`;
...
/*for (const coursesArrayElement of coursesArray) {
    ...
    for ( const itemElements of coursesArrayElement.modules){
        let li = document.createElement('li');
        li.innerText=`${itemElements}`;
        ul.append(li);
    }
...
for (const coursesAndDurationArrayElement of coursesAndDurationArray) {
    let mainDiv=document.createElement('div')
    mainDiv.innerText = `${coursesAndDurationArrayElement.title}--${coursesAndDurationArrayElement.monthDuration}`
    document.body.append(mainDiv)
...
button.onclick = () => {
    ...
    let generateTable = (raws, columns, word) => {
